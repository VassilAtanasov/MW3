# Coding conventions — MW3

> Assembled by `/adopt` from the Ivan conventions reference for this project's stack.
> Judgement rules only. Formatting, style and analyzer severities are owned by `.editorconfig` +
> `Directory.Build.props` and enforced by `gate.ps1` — never argue with them, fix the code.
...
## MW3-specific (MonoGame)

These come from the standing decisions in `docs/ARCHITECTURE.md` and bind every phase.

- **No engine types in `MW3.Core`.** No `Vector2`, `Texture2D`, `GameTime`, or any other MonoGame
  type may appear in the rules library (S-2/D-2). The moment one does, portability is gone and the
  rules stop being testable without a graphics device.
- **Dependency direction is one-way**: `Core` <- `Game` <- platform heads (S-3). If logic *can*
  live in `MW3.Core`, it must. Heads contain wiring only, and are assumed disposable.
- **`MW3.Core` targets `netstandard2.1` with conservative C#.** Unity's language support lags the
  .NET SDK, so avoid the newest language features in `Core` even though `LangVersion` is `latest`.
- **Frame-loop code allocates nothing per frame.** No LINQ, no closures, no `params` arrays, and no
  string concatenation inside `Update`/`Draw`. Allocate at load time and reuse.
- **Deterministic rules**: `MW3.Core` must not read wall-clock time or unseeded randomness. Elapsed
...
moment one is introduced; do not treat their absence as permission to be loose elsewhere.
