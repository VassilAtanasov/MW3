...
| 6 — Forges | `docs/forges/` | A third building type that produces nothing and buffs its owner's attacks and defences *everywhere*, capped at four with diminishing returns, plus an eight-base map with a contested neutral forge and neutral tower — closing parity G-6 and completing G-7. Carries three structural changes a third type forces: explicit-target conversion, an injectable map layout, and the first reachable arithmetic remainder in the composed index |
| 7 — Maps | `docs/maps/` | Three named two-player maps — Small (the phases 2–5 board preserved), Medium (a central obstacle) and Big (two neutral towers guarding a forge) — chosen from three home-screen buttons, plus base shapes shrunk by half (#94). Partly closes parity G-18. Its one new mechanic, **armies routing around obstacles**, is a deliberate divergence from MW2's straight-line movement (`MW2-PARITY.md` §4.1), and it is the first thing to make an army's position something other than a linear interpolation between two bases |
| 8 — Game server | `docs/game-server/` | The simulation moves to an authoritative `MW3.Server` process that runs many matches at once; `MW3.Game` becomes a renderer that draws a `MW3.Protocol` snapshot and submits commands over WebSocket + JSON, with an in-process loopback path keeping offline play and all 50 `qa/scripts/` working. Still one human against the AI — it closes no parity gap and changes no rule. It is where **S-7**'s promise ("never single-player-only by construction") is collected, and it makes **S-8**'s determinism an enforced property rather than an accident (D-60) |
