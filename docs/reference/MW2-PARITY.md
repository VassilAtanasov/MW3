...
| G-18 | Many maps, per campaign mission | One hardcoded six-base layout; **eight bases** after phase 6 FR-2 adds a contested neutral forge and neutral tower, and the layout becomes an injectable value on `Match` (phase 6 D-44) rather than an `internal static` array | Becomes **geolocated maps** (§5). Phase 6 makes the layout *injectable* as a testability seam only — it ships no second map, no map format and no selection UI. A separate future project owns **extended map support**: flexible layouts, paths, obstacles and zones, plus the terrain MW2 implies but never details (Shii'Mori is a terrain-control tribe and Trini's Beehive is documented as *ignoring* terrain, so MW2 has some) |
