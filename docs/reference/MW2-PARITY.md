...
> tapered radius plus shared spine) and a tower's fire is visible as an event. Updated 04-08-2026:
> phase 5, "Morale" (`docs/morale/`), discovered — **G-1** is assigned to it in six features, and
> **G-7**'s morale term with it, leaving G-7 open on the forge term alone. **G-4**, **G-5** and
> **G-16** were deliberately held back to a phase 6, since energy has no sink until heroes exist.
> One correction landed in the same pass: G-1's row claimed morale affects production, and it does
> not — see the row itself. Updated 05-08-2026: phase 5 FR-2 (`f7b795f0a982`, issue #67) merged —
> the morale term is live in the combat formula, closing G-7's row except for the forge term (G-6).
> Updated 05-08-2026: phase 5 FR-3 (`eeb19c449be6`, issue #69) merged, adding inactivity decay.
> Updated again 05-08-2026: FR-4 (`2e35c45de62c`, issue #71) merged, adding unit speed — **G-1
> closes for the rules layer** (FR-1/FR-2/FR-3/FR-4), with the drawn meter owed by FR-5
> (`b0d20abba8ad`), still open — the same treatment G-2 received when phase 4 FR-3 closed its rules
> half and left the column to FR-4.
> Final update 05-08-2026: FR-5 (issue #77, PR #79) and FR-6 (issue #78, PR #80) both merged, so
> **G-1 is fully closed** — rules, drawn meter and morale-aware AI — and the Morale phase is done.
> Updated 06-08-2026: phase 6 FR-3 (`8554c22a4421`, issue #87) merged, making the forge term live in
> every index — **G-7 is fully closed**, and **G-6 closes for the rules layer**, leaving drawing to
> FR-5 (#89) and a forge-aware AI to FR-6.
> Final update 07-08-2026: FR-6 (#93, PR #97) and FR-5 (#89, PR #96) both merged, so **G-6 is fully
> closed** and the Forges phase is done. The same day, phase 7 — **Maps** (`docs/maps/`, wf
> `3f7156d826aa`) — was discovered and **G-18** assigned to it in six features, which partly closes
> that gap (see its row for what stays open). Phase 7 also adds this file's first entry in the new
> **§4.1**: armies routing around obstacles is a deliberate divergence from MW2's straight-line
> movement, agreed by the user rather than researched, so the §1 movement row is annotated rather
> than moved. Heroes (**G-4**), energy (**G-5**) and Rush Mode (**G-16**) — which phase 6's docs had
> pencilled in as phase 7 — slide to phase 8.
...
| Movement | No fog of war. Straight line, base to base, with no pathfinding — **until phase 7 (`docs/maps/`), which adds obstacles armies route around.** That is a deliberate divergence, not a gap: see §4.1 |
...
| # | MW2 | MW3 today | Notes |
|---|---|---|---|
| G-1 | **Morale**: a per-player multiplier, 0–5 suns, worth +125% defence / +25% attack / +50% speed, earned by capturing and defending, drained by losses and by **inactivity on a timer that accelerates as you climb** | **Fully closed.** Phase 5 (`docs/morale/`, wf `3401ecb1c7a5`), discovered 04-08-2026: the score and its gain/loss tables (FR-1, `c99d42cbc681`, issue #66), the combat indices (FR-2, `f7b795f0a982`, issue #67), inactivity decay (FR-3, `eeb19c449be6`, issue #69), and unit speed (FR-4, `2e35c45de62c`, issue #71) are all merged. **Fully closed 05-08-2026** — the drawn meter (FR-5, `b0d20abba8ad`, issue #77, PR #79) and an AI that plays for morale (FR-6, `1713e24400b9`, issue #78, PR #80) both merged, draining board 22 and closing the phase | The largest single system in MW2 and the one every source calls the skill differentiator. Anti-turtle, anti-snowball, and it touches combat and movement at once. **Correction, 04-08-2026:** this row previously said morale touches "combat, speed and production" — it does **not** affect production, in either game. [MW2-RULES.md](MW2-RULES.md) §5.1's table has exactly three effect columns (defence, attack, unit speed) and §3.1 corroborates the speed one independently |
...
| G-5 | **Energy**: 2.5/sec passive plus `0.45 × k` per unit lost attacking, where `k` rises as morale falls — so a losing player earns 5× the energy per casualty | Nothing | The game's rubber band. Depends on G-1 for `k`. **Deliberately paired with G-4 for a future phase 6**, settled with the user 04-08-2026 when phase 5 was scoped: energy is a currency with no sink until hero abilities exist, so shipping it alone would mean a number that accumulates and is spent on nothing. Phase 5's morale state is what `k` reads when it arrives — no rebuild needed |
| G-6 | **Forges**: a third building type giving a global attack/defence buff to its owner, 125–150% defence and 150–200% attack, capping at 4 | Two building types only. **Assigned to phase 6** (`docs/forges/`, wf `3900095949a7`), discovered 05-08-2026: the type and its structural seams (FR-1, `69b8d6032657`), a contested neutral forge and neutral tower on an eight-base map (FR-2, `65f7360af81d`), the count-keyed global buff (FR-3, `8554c22a4421`), the forge morale rows (FR-4, `eb92138da99f`), the menu entry and drawing (FR-5, `06341f0fa15b`), and an AI that contests forges (FR-6, `b78d24560dd7`). **Closed for the rules layer 06-08-2026**: FR-1 (#82), FR-4 (#83), FR-2 (#86) and FR-3 (#87) are merged, so a forge exists, is contested on the shipped map, moves morale, and buys its owner the published global buff. **FR-6 (#93) merged 07-08-2026**: the AI opponent now builds a forge once it owes one (`ForgeTable.ProducersPerForge`), prioritises defending a threatened forge over any threatened non-forge, and its existing attack tiebreak already valued the neutral forge correctly. **Fully closed 07-08-2026**: FR-5 (#89, PR #96) merged the drawing — an upward-pointing triangle on both heads, per-type convert labels and an uncapped `Forges: <n>` count beside each morale meter — so board 23 is drained and phase 6 is complete | Phase 3 §6 forbade a third type *that phase* — sequencing, not design. Feeds the combat formula (G-7), which FR-3 closes. Forge **levels** are not owed by any phase: MW2 gives forges exactly one tier (`MW2-RULES.md` §2, §2.4). The AI's forge heuristics are MW3's own original design (G-21) — no source describes MW2's AI |
| G-7 | **Combat formula** `Bu = (a/d) × Wu`, with building defence, morale and forges stacking into `a` and `d` | **Morale term closed** by phase 5 FR-2 (`f7b795f0a982`, issue #67) 05-08-2026: `CombatResolver.ComposeAttackerIndex`/`ComposeDefenderIndex` read each player's live morale attack/defence percentage from `MoraleTable`, composed at basis-point (1/10000) precision so the two-term product (building defence × morale) is exact with no division loss. Building defence has been live since FR-3b. **Fully closed** by phase 6 FR-3 (`8554c22a4421`, issue #87) 06-08-2026: `CombatResolver.ForgeContributionPercent` is deleted and both Compose methods take a required forge percentage, read from the new `ForgeTable` at each player's live count (derived from the board, never stored - D-45) on the resolve path and both of `AiBrain`'s prediction paths at once. D-46's consequence landed as predicted: three non-identity terms make the basis-point division's remainder reachable for the first time, truncation is kept, and the resulting bias favours the attacker - pinned by name in `ForgeCompositionTests` | Closed once G-6 supplied the forge term - populating it rather than requiring a rewrite, since the composition already handles an arbitrary number of multiplicative terms. **How the terms stack is MW3's assumption, not a parity claim** ([MW2-RULES.md](MW2-RULES.md) §4.3 marks it `[?]` — sources say only "combines"). Settled with the user 04-08-2026 as **multiplicative**, on the evidence that the reference's own worked example multiplies and the shipped `CombatResolver.ComposePercentages` already does. It first *mattered* at phase 5 FR-2, when a defender carries building defence and morale defence as two non-identity terms for the first time. If MW2 is ever observed stacking additively, this reopens — §10's `[?]` stands |
...
| G-18 | Many maps, per campaign mission | One hardcoded layout, six bases and then eight. **Assigned to phase 7** (`docs/maps/`, wf `3f7156d826aa`), discovered 07-08-2026: three named two-player maps — Small (6 slots, the phases 2–5 board preserved), Medium (8 slots plus a central obstacle) and Big (9 slots, two neutral towers with a forge between them) — chosen from three home-screen buttons, with obstacles armies route around | Becomes **geolocated maps** (§5). Phase 6 made the layout *injectable* as a testability seam only; phase 7 is the "extended map support" project that seam was left for, but it closes this gap only **partly**. Still open after it: the map **file format** and any authored map data outside C# (phase 7 D-49 defers it to the Campaigns project), geolocated maps (Branding), per-mission campaign maps, **tunnels and over-the-air hero paths**, zones and any non-blocking terrain, and **N-player layouts** (G-17 — every phase-7 map is strictly two-player). The terrain MW2 implies but never details (Shii'Mori is a terrain-control tribe and Trini's Beehive is documented as *ignoring* terrain) is **not** what phase 7 builds — see §4.1 |
...
| Three base types forbidden | phase 3 §6 | "MW2 has several; this phase earns two" | Reopened as **G-6** (forge); **closed for the rules layer** by phase 6 FR-1/FR-4/FR-2/FR-3 (06-08-2026) |
...
FR-1/FR-2 (`docs/army-sending/`) as of 30-07-2026, the forge (**G-6**) by phase 6 (`docs/forges/`)
as of 06-08-2026, for the rules layer. Each resolution
...
### 4.1 Deliberate divergences agreed in discovery

Differences from MW2 that are **not** gaps and must never be filed as one. §0 forbids recording a
design preference as a §2 row; this is where an agreed difference goes instead, with the date the
user agreed it and the reasoning.

| Divergence | Agreed | Where | Reasoning |
|---|---|---|---|
| **Armies route around obstacles.** MW2 moves strictly straight-line with no pathfinding (§1); MW3 maps may carry obstacles, and a send whose straight line is blocked takes the shortest way past | 07-08-2026, at `/discover Maps` | phase 7, `docs/maps/` (D-55); FR-3 `c4bd0f438bd1` | MW2's terrain behaviour is **unpublished** (`MW2-RULES.md` §10) — its tribes imply terrain exists (Shii'Mori controls it, Trini's Beehive ignores it) but nothing describes what it does to movement. So there is no MW2 answer to copy, and this is MW3's own design, like the AI (G-21). The cheaper alternative — refusing a send whose line is blocked, which would have preserved the straight-line rule exactly — was offered and declined, with the roughly doubled phase cost stated before the choice. Never describe MW3's routing as a port of MW2's |
