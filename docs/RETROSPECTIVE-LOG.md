...
## 2026-08-05 — autopilot run (Morale phase, FR-5 and FR-6, phase close)
- Outcome: 2 shipped (#77 FR-5 morale meter UI, #78 FR-6 AI morale preference — PRs #79, #80), 0 skipped for clarification, 0 circuit-breaker trips, main green on CI. Both features passed code review (APPROVE) and QA verification (VERIFIED) on the first pass, with no re-review or re-verify cycles needed. This closes the Morale project (board 22, phase 5) entirely - all six features (#66, #67, #69, #71, #77, #78) are now shipped and marked complete in Workflowy.
- Went well: both features shipped clean on the first review/QA pass, a contrast with the prior run's session-interruption friction. #77 (pure UI) was additionally verified on the physical Android device beyond the acceptance criteria's headless proof, closing the one device-QA line the automated QA pass explicitly deferred - worth doing whenever a device is attached and a criterion names it, rather than treating "device confirmed" as satisfied by a desktop-only pass. #78's implementer independently found and reused the existing `MoraleTable.AttackingUnitDiedLoss` constant instead of hardcoding the literal `10` the issue's own prose used, correctly reading D-22's "no new literal outside MoraleTable" rule as applying even when the issue text itself states the number directly.
- Caused rework: none within either feature - this run's only friction was external CI/build wall-clock time (Android builds and the shared-worktree gate step each took several minutes), not a code or process defect.
- Follow-ups filed: #81 "AiBrain.TryAttack's full-equality (tower-loss AND morale-swing tied) fallback lacks a regression test" - code-reviewer verified the behavior is already correct by manual trace during #78's review, but flagged the missing test as a gap worth closing before a future refactor could silently invert the tiebreak's short-circuit order.
- Process adjustments applied: none - two clean single-pass features don't show a recurring friction pattern worth a new standing rule.

Morale phase (board 22) complete. Next: run `/discover <next project>` against the Workflowy outline to open the next phase.

## 2026-08-06 — autopilot run (Forges phase, FR-4)
- Outcome: 1 shipped (#83 FR-4 forge morale rows — PR #85), 0 skipped for clarification, 0 circuit-breaker trips, main green on CI. Passed code review (APPROVE, no findings) and QA verification (VERIFIED, every criterion) on the first pass.
- Went well: the qa/scripts/ acceptance criterion demanded an *exact* byte-for-byte derived `Morale:` dump line for a scenario where the AI's own free play (not a scripted human send) does the capturing — a materially harder derivation than prior morale scripts (e.g. morale-capture-neutral.txt), since the AI also conquers four neutral villages and completes a base upgrade before ever reaching the forge. Rather than hand-guess the trajectory, the implementer bisected real desktop-head runs at increasing `wait` values to observe the deterministic intermediate states (no RNG anywhere in this engine), found the four-neutral-conquest phase nets exactly zero morale and the one base upgrade contributes the entire pre-attack baseline, then hand-verified the final two-wave combat arithmetic against `CombatResolver`'s formula — and confirmed the resulting header's prediction matched an actual run exactly, twice, before committing it. This is a reusable technique worth naming: for any future "let the AI's free play prove a scenario" qa script, bisect on `wait` first to map the event timeline before attempting a derivation.
- Caused rework: none - single clean pass, only one issue was ready to build (FR-2/FR-3/FR-5/FR-6 remain un-kicked-off on the Forges board, so the backlog this run drained was just the one issue #82 had already unblocked).
- Follow-ups filed: none - no defects or scope gaps surfaced during this run.
- Process adjustments applied: none - a single clean-build run with a genuinely novel (not recurring) derivation technique doesn't warrant a new standing rule; the bisection technique is recorded here for future reference rather than promoted to CLAUDE.md.

Forges phase (board 23) is **not** complete - only FR-1 (#82) and FR-4 (#83) have been kicked off and shipped; FR-2, FR-3, FR-5, and FR-6 have no GitHub issues yet. `/autopilot`'s backlog (open `feature`-labelled issues on the board) is correctly empty, but that reflects "nothing left that's been kicked off," not "nothing left to build." Next: run `/kickoff` on FR-2 ("The map gains a contested neutral forge and neutral tower", wf `65f7360af81d`) per the build order `docs/forges/REQUIREMENTS.md` records (FR-1 → FR-4 → FR-2 → FR-3 → FR-5 → FR-6).
